...
import com.lh.bot.job.EosBuyJob;
import com.lh.bot.job.EosSaleJob;
...
public class BigMacdMonitorInstance {

    private Logger logger = LoggerFactory.getLogger(BigMacdMonitorInstance.class);
...
//        String diff5;
        {
            List<Double> list = getDataLit(60);
            double[] analyticMACD  = getAnalyticMACD(list);
            if (analyticMACD[0] > 0 && analyticMACD[1] > analyticMACD[0]) {
                logger.info(tag + "60分钟红柱子变大");
                A = "60分钟红柱子变大";
//                diff60 = String.format("%.2f", (analyticMACD[1] - analyticMACD[0]) * 100 / analyticMACD[0]);

            }/* else if (analyticMACD[0] < 0 && analyticMACD[1] < 0 && analyticMACD[1] > analyticMACD[0] ) {
                logger.info(tag + "60分钟绿柱子变小");
                A = "60分钟绿柱子变小";
            }*/
        }
        {
            List<Double> list = getDataLit(10);
            double[] analyticMACD  = getAnalyticMACD(list);
            if (analyticMACD[0] > 0 && analyticMACD[1] > analyticMACD[0]) {
                logger.info(tag + "10分钟红柱子变大");
                B = "10分钟红柱子变大";
